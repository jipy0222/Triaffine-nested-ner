...
class SRLDataset(Dataset):
    def __init__(self, version, mode, tokenizer, truncate_length=128, schema="span", 
                 token_schema="BIES", bert_embed=""):
        self.version = version
        self.mode = mode
        self.schema = schema
    ...
    def _get_file_path(self, version, mode):
        # return f"data/{version}/{mode}_sample.json"
        if version == "conll2012srl":
            return f"data/{version}/conll12{mode}_processed.json"

    def _load_file(self, file_path):
        with open(file_path, "r") as f:
            df = ujson.load(f)
        self.len = len(df)
        smalllen = int(0.002*self.len)
        with open(file_path, "r") as f:
            newdf = ujson.load(f)[:smalllen]
        self.len = len(newdf)
        return newdf
    ...
    def _get_type(self, version):
        if version == "conll2012srl":
            type_list = ['ARG0', 'ARGM-MNR', 'ARG1', 'ARG2', 'ARGM-ADV', 'ARGM-LOC', 'ARGM-TMP',
                        'ARGM-PRP', 'ARGM-PRD', 'R-ARGM-TMP', 'ARGM-DIR', 'ARGM-DIS', 'ARGM-MOD',
                        'R-ARGM-LOC', 'ARG4', 'R-ARG1', 'R-ARG0', 'ARG3', 'ARGM-NEG', 'ARGM-CAU',
                        'C-ARG1', 'ARGM-EXT', 'C-ARGM-ADV', 'ARGM-LVB', 'ARGM-REC', 'R-ARG3', 'R-ARG2',
                        'C-ARG0', 'ARGM-ADJ', 'C-ARG2', 'R-ARGM-CAU', 'R-ARGM-DIR', 'ARGM-GOL', 'ARGM-DSP',
                        'ARGM-PRR', 'R-ARGM-ADV', 'ARGA', 'R-ARGM-MNR', 'ARGM-COM', 'ARGM-PRX', 'R-ARG4',
                        'C-ARGM-LOC', 'C-ARG4', 'ARGM-PNC', 'ARG5', 'C-ARGM-PRP', 'C-ARGM-TMP', 'C-ARGM-MNR',
                        'C-ARGM-DSP', 'C-ARG3', 'R-ARGM-COM', 'R-ARGM-PRP', 'R-ARGM-GOL', 'C-ARGM-EXT',
                        'R-ARGM-EXT', 'C-ARGM-ADJ', 'R-ARGM-PNC', 'C-ARGM-DIS', 'C-ARGM-NEG', 'C-ARGM-COM',
                        'C-ARGM-DIR', 'R-ARGM-MOD']
        return {tp: idx for idx, tp in enumerate(type_list)}
    ...
    # def convert_verb(self, verb):
    #     new_verb = np.full(self.max_word_count, 0, dtype='int64')
    #     new_verb[verb-1] = 1
    #     return new_verb.tolist()
    ...
    def convert(self, word_count, entities):
        ...
        if self.schema in ['span']:
            ...
            for ent in entities:
                if ent['end'] - 1 < self.max_word_count:
                    new_label[ent['start']-1, ent['end']-1] = self.type2id[ent['type']]
            return new_label.tolist(), None, None, None
    ...
    def __getitem__(self, index):
        ...
        span_label, head_label, tail_label, ori_label = self.convert(word_count, argument)
        token_label = self.convert_token(word_count, argument)
        head_trans_label, tail_trans_label = self.convert_trans(word_count, argument)
        ...
        return input_ids, attention_mask, ce_mask, token_type_ids, subword_group, \
               ...
               r_input_word, r_input_char, r_input_pos, \
               span_label, token_label, head_label, tail_label, ori_label, \
               head_trans_label, tail_trans_label, verb, bert_embed


def srl_collate_fn(batch):
    type_count = len(batch[0])
    batch_size = len(batch)
    output = ()
    for i in range(type_count):
...
def get_cls_num_list(task, version):
    if task == "ner":
        mode = 'train'
        if version == "ace04":
        ...
        mode = 'train'
        if version == "conll2012srl":
            file_path = f"data/conll2012srl/conll12{mode}_processed.json"
        ...
        for now_df in df:
            n = len(now_df['tokens'])
            count_list[-1] += n * (n + 1) // 2 - len(now_df['argument'])
            for ent in now_df['argument']:
                count_list[type_list.index(ent['type'])] += 1
        return count_list
